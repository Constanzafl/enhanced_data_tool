...
class SmartRelationshipDetector:
    def __init__(self, tables: Dict[str, pd.DataFrame]):
        ...
        self.primary_keys = {}  # Nuevo: almacenar PKs detectadas
    ...
    def _calculate_name_similarity(self, source: ColumnProfile, target: ColumnProfile) -> float:
        ...
        # Ejemplo: pets.patientIdentifier → patients.uid
        if target_is_pk and not source_is_pk:
        ...
        # Si ambos tienen componentes ID y comparten palabras base
        ...
        # Ejemplo: "PatientUniqueIdentifier" vs "PersonUID"
...
, r'^id
    ...
    def analyze_columns(self):
        ...
        for table_name, df in self.tables.items():
            ...
            for column in df.columns:
                profile = self._create_column_profile(table_name, df, column)
                self.column_profiles[f"{table_name}.{column}"] = profile
                print(f"  - {column}: {profile.data_type}, "
                      f"{profile.unique_count} únicos, "
                      f"{profile.null_count} nulos")
    ...
    def _calculate_name_similarity(self, source: ColumnProfile, target: ColumnProfile) -> float:
        ...
        # REGLA 1: Evitar relacionar PKs genéricas entre tablas diferentes
        # pets.id NO debe relacionarse con patients.id
        if source_name == 'id' and target_name == 'id':
            return 0.0  # Las PKs de diferentes tablas NO están relacionadas
        
        # REGLA 2: Patrón clásico de FK: tabla.otratabla_id → otratabla.id
        # Ejemplo: pets.patient_id → patients.id
        if target_name == 'id' or target_name.endswith('_id'):
            # Extraer el nombre base de la columna source
            for pattern in self.common_id_patterns:
                source_base = re.sub(pattern, '', source_name)
                if source_base:
                    # Verificar si source_base coincide con el nombre de la tabla target
                    # pets.patient_id (source_base='patient') → patients.id
                    if (target_table.startswith(source_base) or 
                        source_base.startswith(target_table.rstrip('s')) or
                        source_base + 's' == target_table):
                        return 0.95  # Alta confianza
                    
                    # Verificar mapeos conocidos
                    for concept, variations in self.common_name_mappings.items():
                        if source_base in variations:
                            for var in variations:
                                if target_table.startswith(var) or var.startswith(target_table.rstrip('s')):
                                    return 0.85
        
        # REGLA 3: Patrón inverso: otratabla.id → tabla.otratabla_id
        if source_name == 'id' or source_name.endswith('_id'):
            for pattern in self.common_id_patterns:
                target_base = re.sub(pattern, '', target_name)
                if target_base:
                    # patients.id → pets.patient_id
                    if (source_table.startswith(target_base) or 
                        target_base.startswith(source_table.rstrip('s')) or
                        target_base + 's' == source_table):
                        return 0.95
        
        # REGLA 4: Nombres idénticos (pero no "id")
        if source_name == target_name and source_name != 'id':
            return 0.9  # Alta confianza pero no total
        
        # REGLA 5: Evitar falsos positivos con columnas genéricas
        generic_columns = ['id', 'name', 'description', 'created_at', 'updated_at', 'status']
        if source_name in generic_columns and target_name in generic_columns:
            return 0.1  # Muy baja confianza para columnas genéricas
        
        # REGLA 6: Verificar si uno contiene al otro (excluyendo casos genéricos)
        if source_name != 'id' and target_name != 'id':
            if source_name in target_name or target_name in source_name:
                return 0.7
        
        # REGLA 7: Similitud de cadenas general (solo si no son genéricas)
        if source_name not in generic_columns or target_name not in generic_columns:
            similarity = SequenceMatcher(None, source_name, target_name).ratio()
            if similarity > 0.8:
                return similarity * 0.8  # Reducir un poco la confianza
...
if __name__ == "__main__":
    ...
    pass
        ...
        self.common_name_mappings = {
            'patient': ['patient', 'person', 'individual', 'client'],
            'appointment': ['appointment', 'visit', 'booking', 'schedule'],
            'medication': ['medication', 'medicine', 'drug', 'prescription'],
            'doctor': ['doctor', 'physician', 'provider', 'staff'],
            'pet': ['pet', 'animal', 'patient']  # Para veterinarias
        }
